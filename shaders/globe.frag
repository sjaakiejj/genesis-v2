...
uniform float show_borders;     // Toggle border visibility (0.0 or 1.0)
...
void main() {
    ...
    // 4. Highlight Logic (only if borders are shown)
    if (is_selected > 0.5 && show_borders > 0.5) {
        // Boost brightness for selected plate
        color.rgb *= 1.4;
    }

    // 5. Border Detection (for all plates - black borders)
    // Only apply if show_borders is enabled
    if (show_borders > 0.5) {
        float border = 0.0;
